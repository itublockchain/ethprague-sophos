...
export const getAddressFromPublicKey = (publicKey: string): string => {
  const formattedKey = publicKey.startsWith("0x")
    ? publicKey
    : `0x${publicKey}`;
...
export class WebSocketClient {
  ...
  get readyState(): WebSocketReadyState {
    return this.ws
      ? (this.ws.readyState as WebSocketReadyState)
  ...
  private async authenticate(): Promise<void> {
    ...
    if (!walletClient?.account?.address)
      throw new Error("Wallet client not initialized or address not available");
    ...
    if (jwtToken) {
      ...
      console.log(
        "No JWT token found, proceeding with challenge-response authentication"
      );
      ...
      const handleAuthResponse = async (event: MessageEvent) => {
        ...
        try {
          if (response.res && response.res[1] === "auth_challenge") {
            ...
            const eip712SigningFunction = createEIP712SigningFunction(
              this.signer
            );
  ...
  private handleMessage(event: MessageEvent): void {
    ...
    try {
      ...
      if (
        hasProperty(message, "res") &&
        Array.isArray(message.res) &&
        message.res.length >= 3
      ) {
        const requestId =
          typeof message.res[0] === "number" ? message.res[0] : -1;
      ...
      if (
        hasProperty(message, "err") &&
        Array.isArray(message.err) &&
        message.err.length >= 3
      ) {
        const requestId =
          typeof message.err[0] === "number" ? message.err[0] : -1;
      ...
      if (hasProperty(message, "type") && typeof message.type === "string") {
        ...
        if (
          hasProperty(message, "requestId") &&
          typeof message.requestId === "number"
        ) {
          const requestId = message.requestId;
          if (this.pendingRequests.has(requestId)) {
            const result = hasProperty(message, "data")
              ? message.data
              : message;
