...
export const WebSocketProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const generateKeys = useCallback(async () => {
    try {
      ...
      const errorMsg = `Error generating keys: ${
  ...
  useEffect(() => {
    if (currentSigner && !clientRef.current) {
      ...
      console.log(
        "WebSocket client initialized, waiting for MetaMask connection..."
      );
    }
  ...
  const connect = useCallback(async () => {
    if (!clientRef.current) {
      console.error(
        "Cannot connect: WebSocket client not initialized (no signer?)"
      );
      return false;
    }
    if (clientRef.current.isConnected) {
    ...
    if (!walletClient?.account?.address) {
      console.log(
        "Cannot connect to WebSocket: MetaMask wallet not connected. Please connect MetaMask first."
      );
      return false;
    }
  ...
  const setNitroliteChannel = useCallback((nitroliteChannel: Channel) => {
    ...
    console.log(
      `Set Nitrolite channel: ${JSON.stringify(nitroliteChannel).slice(
        0,
        50
      )}...`
    );
