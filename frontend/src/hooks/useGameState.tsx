...
import { useAppSessionSignature } from "@/hooks/useAppSessionSignature";
...
export function useGameState(
  ...
  const [pendingSignatureRequest, setPendingSignatureRequest] = useState<
    AppSessionSignatureRequestMessage | AppSessionStartGameRequestMessage | null
  >(null);
  ...
  const setCurrentStatus = useSetAtom(currentStatusAtom);

  // Determine player's role (X or O)
  const playerSymbol =
    gameState.players.X === eoaAddress
      ? "X"
      : gameState.players.O === eoaAddress
      ? "O"
      : null;
  ...
  useEffect(() => {
    ...
    switch (lastMessage.type) {
      ...
      case "appSession:signatureRequest":
        console.log(
          "Received signature request for participant B:",
          lastMessage
        );
        setPendingSignatureRequest(
          lastMessage as AppSessionSignatureRequestMessage
        );
      ...
      case "appSession:startGameRequest":
        console.log(
          "Received start game request for participant A (host):",
          lastMessage
        );
        setPendingSignatureRequest(
          lastMessage as AppSessionStartGameRequestMessage
        );
  ...
  const signAndStartGame = useCallback(async () => {
    if (
      ...
      pendingSignatureRequest.type !== "appSession:startGameRequest"
    ) {
    ...
    try {
      await handleParticipantASignature(
        pendingSignatureRequest as AppSessionStartGameRequestMessage
      );
      setPendingSignatureRequest(null);
