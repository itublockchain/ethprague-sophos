...
export function useChannel() {
  ...
  const { currentNitroliteChannel, setNitroliteChannel } =
    useWebSocketContext();
   ...
   */
  const saveChannelToStorage = useCallback(
    (state: State, channelId: string) => {
      try {
    ...
    async (tokenAddress: Hex, amount: string) => {
      ...
      try {
        ...
        if (existingChannel.exists) {
          ...
          let message =
            "Cannot create a new channel because one already exists.";
          ...
          if (source === "accountChannels") {
            ...
            message +=
              " Please close the existing channel before creating a new one.";
          }
        ...
        if (!client) {
          console.error(
            "Nitrolite client not initialized - client is null in store"
          );
   ...
   */
  const depositToChannel = useCallback(
    async (tokenAddress: Address, amount: string) => {
      ...
      try {
        ...
        if (!client) {
          console.error(
            "[depositToChannel] Nitrolite client not initialized - client is null in store"
          );
          throw new Error("Nitrolite client not initialized");
        }
        ...
        console.log(
          "[depositToChannel] Available client methods:",
          Object.keys(client)
        );
