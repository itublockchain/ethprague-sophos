...
export function useAppSessionSignature(
  ...
  const signAppSessionMessage = useCallback(
    async (
      ...
      messageType: "appSession:signature" | "appSession:startGame"
    ) => {
      ...
      try {
        ...
        console.log(
          "Client signing request structure:",
          JSON.stringify(requestToSign, null, 2)
        );
        ...
        if (messageType === "appSession:signature" && sendSignature) {
          ...
          throw new Error(
            "No send function available for message type: " + messageType
          );
        }
