...
import { handleStartGame, handleBet } from "./routes/gameRoutes.js";
import {
 ...
 */
async function handleAppSessionSignature(
  ws,
  payload,
  ...
  if (!roomId || !signature) {
    return sendError(
      ws,
      "INVALID_PAYLOAD",
      "Room ID and signature are required"
    );
  }
  ...
  try {
    const allSignaturesCollected = await addAppSessionSignature(
      roomId,
      playerEoa,
      signature
    );
    ...
    if (room && playerEoa === room.players.guest && !allSignaturesCollected) {
      logger.nitro(
        `Participant B signed, now requesting signature from participant A (host)`
      );
      ...
      if (hostConnection && hostConnection.ws.readyState === 1) {
        ...
        logger.error(
          `Host connection not found or not ready for room ${roomId}`
        );
      }
    }
    ...
    if (allSignaturesCollected) {
      logger.nitro(
        `All signatures collected for room ${roomId}, creating app session`
      );
      // The app session creation will be handled by the handleAppSessionStartGame function
    }
  } catch (error) {
    logger.error(
      `Error handling app session signature for room ${roomId}:`,
      error
    );
 ...
 */
async function handleAppSessionStartGame(
  ws,
  payload,
  { roomManager, connections, sendError }
) {
  ...
  try {
    ...
    if (!allSignaturesCollected) {
      return sendError(
        ...
        "Not all signatures collected"
      );
    }

    logger.nitro(
      `Host signature added for room ${roomId}, creating app session`
    );
    ...
    logger.error(
      `Error handling app session start game for room ${roomId}:`,
      error
    );
    ...
    try {
      switch (data.type) {
        ...
        case "startGame":
          ...
          break;
        case "bet":
          await handleBet(ws, data.payload, context);
          break;
